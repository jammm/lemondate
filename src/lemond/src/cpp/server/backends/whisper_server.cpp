...
InstallParams WhisperServer::get_install_params(const std::string& backend, const std::string& version) {
    ...
    if (backend == "npu") {
        ...
        // No prebuilt ROCm whisper-server is published — users supply one
        // via LEMONADE_WHISPERCPP_ROCM_BIN (e.g. built from
        // ggml-org/whisper.cpp with GGML_HIP=ON). Returning empty repo/
        // filename is fine because install_from_github short-circuits as
        // soon as find_external_backend_binary() returns a path.
        params.repo = "";
        params.filename = "";
    } else {
...
void WhisperServer::load(const std::string& model_name,
    ...
    // get_device_type_from_recipe() defaults whispercpp to CPU, but npu/vulkan/rocm use different devices.
    if (whispercpp_backend == "npu") {
        device_type_ = DEVICE_NPU;
    } else if (whispercpp_backend == "vulkan" || whispercpp_backend == "rocm") {
