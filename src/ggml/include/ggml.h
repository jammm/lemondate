...
#    endif
    ...
    GGML_API struct ggml_tensor * ggml_cumsum_tts(
            struct ggml_context * ctx,
            struct ggml_tensor  * a);
    // Snake activation (ttscpp). Computes f(a) = a + sin^2(a*alpha) / alpha
    // element-wise, broadcasting alpha across a along ne[0]. Replaces a
    // 7-op subgraph (mul, sin, sqr, mul, div, mul, add) with a single
    // fused kernel — the per-call kernel-launch latency was the
    // dominant cost on koboldcpp's HIP path for Kokoro before this.
