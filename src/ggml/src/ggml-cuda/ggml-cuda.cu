...
#include "ggml-cuda/ttscpp_ops.cuh"
...
static bool ggml_cuda_compute_forward(ggml_backend_cuda_context & ctx, struct ggml_tensor * dst) {
    switch (dst->op) {
        ...
        case GGML_OP_RECIPROCAL:
            ggml_cuda_op_reciprocal(ctx, dst);
            break;
        case GGML_OP_TTSROUND:
            ggml_cuda_op_ttsround(ctx, dst);
            break;
        case GGML_OP_MOD:
            ggml_cuda_op_mod(ctx, dst);
            break;
...
static bool ggml_cuda_graph_check_compability(ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        // Upstream had `ADD with src[1]->ne[1] > 1` as a graph-disabling
        // condition. Empirically that disables HIP graphs for ~every
        // ttscpp model (Kokoro alone has thousands of residual /
        // bias ADDs whose src[1] has ne[1] > 1) and the bin_bcast
        // kernel handles those shapes correctly. Keep the override
        // gated by GGML_DISABLE_BCAST_ADD_GRAPHS so the original
        // behaviour is one env var away if a regression shows up
        // somewhere.
        if (getenv("GGML_DISABLE_BCAST_ADD_GRAPHS") &&
            node->op == GGML_OP_ADD && node->src[1] && node->src[1]->ne[1] > 1) {
            use_cuda_graph = false;
        }
