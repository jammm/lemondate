...
bool ggml_cuda_should_use_mmf(enum ggml_type type, int cc, int warp_size, const int64_t * src0_ne,
    ...
    switch (type) {
        case GGML_TYPE_F32:
            // Upstream only enables F32 mmf for Ampere+ / MFMA (CDNA).
            // I tried adding amd_wmma_available here for RDNA3/RDNA4
            // to dodge TheRock's incomplete rocBLAS Tensile path on
            // gfx1201 — it hangs the koboldcpp process indefinitely
            // on the first F32 mmf launch. Probably the F32 mmf
            // kernel templates aren't compiling out the MFMA-specific
            // intrinsics for non-MFMA targets. Revisit when upstream
            // adds an explicit RDNA path or when our build instances
            // the kernel for amd_wmma_available too.
