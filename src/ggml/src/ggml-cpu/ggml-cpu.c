...
// Snake activation: dst[i,j,k,l] = a[i,j,k,l] + sin^2(a[i,j,k,l] * alpha[0,j,...]) / alpha[0,j,...].
// alpha is broadcast across ne[0] (and across ne[2]/ne[3] if its dims
// there are 1). Threading splits along the row axis (ne[1] * ne[2] *
// ne[3]); each thread sweeps the contiguous ne[0] dim row-at-a-time.
static void ggml_compute_forward_snake_1d_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {

    const struct ggml_tensor * src0 = dst->src[0];  // a
    const struct ggml_tensor * src1 = dst->src[1];  // alpha

    const int ith = params->ith;
    const int nth = params->nth;

    GGML_TENSOR_BINARY_OP_LOCALS;
    GGML_ASSERT(ne00 == ne0 && ne01 == ne1 && ne02 == ne2 && ne03 == ne3);
    GGML_ASSERT(ne10 == 1);
    GGML_ASSERT(ne11 == ne1);
    GGML_ASSERT(ne12 == 1 || ne12 == ne2);
    GGML_ASSERT(ne13 == 1 || ne13 == ne3);
    GGML_ASSERT(nb00 == sizeof(float));
    GGML_ASSERT(nb0  == sizeof(float));
    GGML_ASSERT(nb10 == sizeof(float));

    const int64_t total_rows = ne1 * ne2 * ne3;
    const int64_t per_thread = (total_rows + nth - 1) / nth;
    const int64_t r0 = ith * per_thread;
    const int64_t r1 = MIN(r0 + per_thread, total_rows);
    ...
    for (int64_t r = r0; r < r1; r++) {
        ...
        const int64_t j2 = (ne12 == 1) ? 0 : i2;
        const int64_t j3 = (ne13 == 1) ? 0 : i3;

        const float * a_row =
            (const float *)((const char *) src0->data + i1 * nb01 + i2 * nb02 + i3 * nb03);
        float * d_row =
            (float *)((char *) dst->data + i1 * nb1 + i2 * nb2 + i3 * nb3);
        const float al =
            *(const float *)((const char *) src1->data + i1 * nb11 + j2 * nb12 + j3 * nb13);
        const float inv_al = 1.0f / al;

        for (int64_t i0 = 0; i0 < ne0; i0++) {
            const float a = a_row[i0];
            const float s = sinf(a * al);
            d_row[i0] = a + s * s * inv_al;
        }
    }
}
static void ggml_compute_forward_snake_1d(
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_snake_1d_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
