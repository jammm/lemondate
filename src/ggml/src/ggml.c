...
struct ggml_tensor * ggml_snake_1d(
    ...
    // alpha must broadcast over a's first dim (snake activation runs
    // per-channel: alpha is (1, channels, ...) and a is (length,
    // channels, ...)). Other dims must match exactly.
    GGML_ASSERT(alpha->ne[0] == 1);
    GGML_ASSERT(a->ne[1] == alpha->ne[1]);
    GGML_ASSERT(a->ne[2] == alpha->ne[2] || alpha->ne[2] == 1);
    GGML_ASSERT(a->ne[3] == alpha->ne[3] || alpha->ne[3] == 1);
    GGML_ASSERT(a->type == GGML_TYPE_F32 && alpha->type == GGML_TYPE_F32);
    struct ggml_tensor * result = ggml_dup_tensor(ctx, a);
