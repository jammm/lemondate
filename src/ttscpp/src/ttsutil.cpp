...
struct ggml_tensor * reciprocal(ggml_context * ctx, struct ggml_tensor * x) {
    TTS_ASSERT(x->ne[0] == 1);
    // Compute 1/x as x / (x*x). The previous implementation built a
    // "ones" tensor by setting tensor->data to a host-side `static
    // constexpr float one` and using stride 0 for broadcast — that
    // CPU pointer ends up being dereferenced by the GGML scheduler's
    // CUDA/HIP kernel when the graph is placed on the GPU, producing
    // an immediate "ROCm error: unspecified launch failure" at the
    // first reciprocal-using op (snake_1d on the noise/resblock alphas).
    // x / (x*x) is mathematically equivalent for x != 0 (alphas are
    // learnable parameters, never zero), uses only standard ggml ops
    // that have GPU implementations, and lets the scheduler manage
    // tensor placement properly. Costs one extra MUL per call vs the
    // original.
    return ggml_div(ctx, x, ggml_mul(ctx, x, x));
}

// Described in https://arxiv.org/abs/2006.08195
// Snake1d is a common tunable activation function used in the DAC model.
//
// Was implemented as a 7-op subgraph (mul, sin, sqr, mul, div, mul,
// add — the inner reciprocal expanded to mul/div). On koboldcpp's HIP
// path that meant 7 kernel launches per call and Kokoro calls snake_1d
// many times per inference (every AdaIN res block + every noise res
// block), so it dominated the per-op launch latency. ggml_snake_1d is
// a single fused kernel with the same semantics — see ggml.h for the
// declaration and ggml-cuda/snake.cu / ggml-cpu.c for the
// implementations.
struct ggml_tensor * snake_1d(ggml_context * ctx, struct ggml_tensor * alpha, struct ggml_tensor * a) {
    assert(a->ne[2] == 1 && a->ne[3] == 1);
    return ggml_snake_1d(ctx, a, alpha);
}
