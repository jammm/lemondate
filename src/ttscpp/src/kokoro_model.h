...
struct kokoro_model : tts_model {
	...
	// CPU-resident copy of the iSTFT window. The window tensor itself
	// lives in the model buffer (which is on the GPU when cpu_only=
	// false), but compute_window_squared_sum is a CPU helper that runs
	// in set_inputs(), before graph compute, and needs raw CPU access
	// to the window samples. Without caching we'd be reading through a
	// GPU device pointer from CPU code -> access violation.
	std::vector<float> window_cpu_cache;
