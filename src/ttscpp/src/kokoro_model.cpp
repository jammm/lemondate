...
void kokoro_model::post_load_assign() {
	...
	// Same alignment requirement as tts_model::set_tensor — every
	// tensor placement must respect the backend's alignment, otherwise
	// HIP kernels touching these post-load constants (n_kernels_tensor
	// in build_sin_gen, harmonic_sampling_norm in build_sin_gen,
	// stft_window in stft, etc.) crash with "unspecified launch
	// failure" the moment the scheduler dispatches to them.
	const size_t pl_align = ggml_backend_buft_get_alignment(buffer);
	auto align_offset = [&]() {
		if (pl_align > 1) {
			offset = (offset + pl_align - 1) & ~(pl_align - 1);
		}
	};
	...
	if (window == "hann") {
    	...
    	offset += size;
		// Keep a CPU copy for compute_window_squared_sum (see header).
		window_cpu_cache = std::move(wdata);
	} else {
	...
	offset += scsize;
	// Pad post_load_tensor_bytes to cover the worst-case alignment
	// padding we just consumed (so the buffer-allocation check in
	// prep_buffers_and_context's add_on_size has headroom).
	post_load_tensor_bytes = 300 + (offset - original_offset) + pl_align * 8;
}
...
void kokoro_duration_runner::set_inputs(kokoro_ubatch & batch) {
	...
	// Same dance as kokoro_runner::set_inputs — build CPU staging
	// buffers and upload via ggml_backend_tensor_set so this works
	// when the scheduler placed positions / attn_mask on a GPU buffer.
	std::vector<int32_t> pos(batch.n_tokens);
	for (uint32_t i = 0; i < batch.n_tokens; i++) {
		pos[i] = (int32_t) i;
	}
	ggml_backend_tensor_set(kctx->positions, pos.data(), 0,
		batch.n_tokens * ggml_element_size(kctx->positions));

	std::vector<float> attn((size_t) batch.n_tokens * batch.n_tokens, 0.0f);
	// Kokoro doesn't use causal attention (it's not autoregressive), so
	// the mask is just zeros — same as the original code, just built in
	// a CPU vector first.
	ggml_backend_tensor_set(kctx->attn_mask, attn.data(), 0,
		attn.size() * ggml_element_size(kctx->attn_mask));
}
...
void kokoro_runner::set_inputs(kokoro_ubatch & batch, uint32_t total_size) {
	// Build all input tensor data in CPU staging buffers, then upload via
	// ggml_backend_tensor_set so this works regardless of whether the
	// scheduler placed the tensors on CPU or GPU memory.
	const size_t uv_n = (size_t) total_size * model->up_sampling_factor * (model->harmonic_num + 1) + 4;
	std::vector<float> uv(uv_n);
	uv[0] = model->voice_threshold;
	uv[1] = model->noise_std;
	uv[2] = model->sin_amp;
	uv[3] = model->sin_amp / 3.0f;
	random_uniform_gen((int) (uv_n - 4), uv.data() + 4);
	ggml_backend_tensor_set(kctx->uv_noise_data, uv.data(), 0,
		uv_n * ggml_element_size(kctx->uv_noise_data));

	const size_t wsq_n = (size_t) total_size * model->up_sampling_factor;
	std::vector<float> wsq(wsq_n);
	// model->window_cpu_cache is populated in post_load_assign; the
	// model weight tensor itself may live on GPU now, so we read from
	// the cached CPU vector instead of model->decoder->generator->window
	// ->data (which would dereference a device pointer).
	compute_window_squared_sum(model->true_n_fft, model->stft_hop,
		total_size * model->up_sampling_factor / model->stft_hop,
		wsq.data(), model->window_cpu_cache.data());
	ggml_backend_tensor_set(kctx->window_sq_sum, wsq.data(), 0,
		wsq_n * ggml_element_size(kctx->window_sq_sum));
	...
	const size_t dm_n = (size_t) batch.n_tokens * total_size;
	std::vector<float> dm(dm_n, 0.0f);
	float running = 0;
	for (uint32_t i = 0; i < batch.n_tokens; i++) {
		float next_running = running + batch.resp->lengths[i];
		for (uint32_t ii = 0; ii < total_size; ii++) {
			dm[i*total_size+ii] = ii >= running && ii < next_running ? 1.0f : 0.0f;
		}
		running = next_running;
	}
	ggml_backend_tensor_set(kctx->duration_mask, dm.data(), 0,
		dm_n * ggml_element_size(kctx->duration_mask));
}
...
#ifdef GGML_USE_CUDA
    // Reuse the model's backend instance so model + duration + audio
    // share ONE HIP stream. Empirically, calling
    // ggml_backend_cuda_init(0) multiple times produces independent
    // streams whose interleaving crashes mid-MUL with "ROCm error:
    // unspecified launch failure" on gfx1201 / TheRock 7.x. The
    // ownership note: model->free() owns ggml_backend_free; we mark
    // owned_backend=false here so the runner_context destructor knows
    // not to double-free.
    if (!use_cpu && model->backend != nullptr) {
        kctx->backend          = model->backend;
        kctx->backend_buffer   = ggml_backend_cuda_buffer_type(0);
